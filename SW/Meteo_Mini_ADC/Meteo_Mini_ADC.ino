...
void loop() {
  float unadjusted_voltage = (analogRead(ADC_PIN) / 4095.0 * 4.423);
  float adjusted_voltage   = ReadVoltage(ADC_PIN);
  
  Serial.println("      Input Voltage = 4.026 v");
  Serial.println("------------------------------");
  Serial.println("   Adjusted Voltage = " + String(adjusted_voltage, 3) + "v  " + String(adjusted_voltage / 4.026 * 100 - 100) + "% error");
  Serial.println("Un-adjusted Voltage = " + String(unadjusted_voltage, 3) + "v  " + String(unadjusted_voltage / 4.026 * 100 - 100) + "% error");
  Serial.println();
  delay(5000);
}
...
float ReadVoltage(byte ADC_Pin) {
  ...
  return (analogRead(ADC_PIN) / 4095.0) * 4.423 * (1100 / vref) * calibration;  // ESP by design reference voltage in mV
}
